...
extern "C"
{
    ...
    //__declspec(dllexport) BOOL ZgReadGPTHeader(PCHAR DiskName, PZG_GPT_HEADER gptheader);
...
/*BOOLEAN ZgGetDriveSerialNumber(PCHAR Drive, PCHAR outbuffer, size_t outbuffersize)
{
    if (Drive == NULL) return FALSE;
    if (outbuffer == NULL) return FALSE;
    ...
    DriveQueryResult = (PSTORAGE_DEVICE_DESCRIPTOR)tmp;
    sprintf_s(outbuffer, outbuffersize, "%s", 
        DriveQueryResult->SerialNumberOffset != 0 ? (const char *)(tmp + DriveQueryResult->SerialNumberOffset):(""));
